...
function User_phone_num_login({ navigation }) {
  ...
  const send_auth_number = async (phone_num) =>{
    try {
      ...
      const verification_id = await provider.verifyPhoneNumber(phone_num, FirebaseRecaptchaRef.current);
      if(verification_id){
  ...
  return (
    <SafeAreaView style={globalStyles.view_screen}>
      ...
      <FirebaseRecaptchaVerifierModal ref={FirebaseRecaptchaRef} firebaseConfig={firebaseConfig} />
        <View>
