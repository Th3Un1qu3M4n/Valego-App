...
function Worker_vehicle_requested({ navigation }) {
  ...
  const onBtnClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/worker/vehicleReady/${request.requestId}`,{}, {
        headers,
      });
  ...
  return (
    <SafeAreaView style={[globalStyles.view_screen,{height:"100%"}]}>
      <Header />
      <View >
        <Text style={globalStyles.text_label_heading}>Vehicle requested for delivery </Text>
        <Image
        ...
        < TouchableOpacity onPress={onBtnClick}>
          <View style={[globalStyles.btn_01, {flexDirection:"row", justifyContent:"flex-start", paddingHorizontal:15}]}>
            <Image
              ...
              style={styles.icon}
            />
            <Text style={[globalStyles.text_label_btn01,{marginLeft:15}]}>Vehicle Ready</Text>
          </View>
        </TouchableOpacity>
        < TouchableOpacity onPress={handleDialPress}>
          <View style={[globalStyles.btn_01, {flexDirection:"row", justifyContent:"flex-start", paddingHorizontal:15, marginVertical:0}]}>
            <Image
              source={require(`../../../../../assets/icons/cuwhite.png`)}
              style={styles.icon}
            />
            <Text style={[globalStyles.text_label_btn01,{marginLeft:15}]}>Contact Owner</Text>
          </View>
        </TouchableOpacity>
